---
title: "Guía de Extensión y Creación de Plugins: dietrich"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de dietrich"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-dietrich-plugins)=
# Guía de Extensión y Plugins: dietrich

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`dietrich`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-dietrich-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`dietrich` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para dietrich`.
- **Punto de Entrada Oficial**: `dietrich.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-dietrich-plugins-tutorial)=
...
class CustomPlugin:
    """Plugin de extensión para dietrich."""
    name = "custom_rule"
...
Para que `dietrich` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."dietrich.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `dietrich` la reconozca:
...
dietrich plugins list
````

---

(manual-dietrich-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `dietrich` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = dietrich.ejecutar_analisis(
...
(manual-dietrich-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `dietrich` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar dietrich
        run: pip install -e ./dietrich
        
      - name: Ejecutar Auditoría
        run: dietrich check src/ include/ --json > reporte.json
````

---

(manual-dietrich-plugins-ejercicios)=
